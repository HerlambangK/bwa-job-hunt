...
async function getDetailJob(id: string) {
  ...
  const isApplay = await prisma.applicants.count({
    where: {
      userId: session?.user.id,
    },
  });

  const benefit: any = data?.benefits;
    ...
    <>
      <div className="bg-slate-100 px-32 pt-10 pb-14">
        ...
        <div className="bg-white shadow mt-10 p-5 w-11/12 mx-auto flex flex-row justify-between items-center">
          ...
          {session ? (
            <>
              {data?.isApplay === 1 ? (
                <Button disabled className="text-lg p-12 py-6 bg-green-500">
                  Applyed
                </Button>
              ) : (
                <FormModalApply
        ...
        <div className="w-1/4">
          <div>
            ...
            <div className="mt-6 p-4 bg-slate-50 rounded-lg">
              ...
              <Progress value={(data.applicant / data.needs) * 100} />
            </div>
      ...
      <div className="px-32 pb-16 ">
        ...
        <div className="grid grid-cols-5 gap-3">
          {data?.benefits?.map((item: any, key: number) => (
            <div key={item + key}>
